...
def fetch_gex_data(
    ...
    # If today's snapshot isn't ready yet, fall back to the previous trading day
    if not expiry_dates:
        prev_date = get_last_market_open(data_date - timedelta(days=1))
        logger.warning("No expirations for %s — retrying with previous trading day %s",
                       data_date.isoformat(), prev_date.isoformat())
        data_date    = prev_date
        expiry_dates = api.get_expirations(ticker, limit=n_expiries, as_of_date=data_date)
    if not expiry_dates:
